	...
	while (ddx < TESTCOUNT)
	{
		...
		while (sdx < TESTCOUNT)
		{
			...
			while (sidx < TESTCOUNTSIZE)
			{
				...
				orig_ret = memmove(sandboxorig+testdst[ddx][0],sandboxorig+testsrc[sdx][0],testsize[sidx]);
				test_ret = ft_memmove(sandboxtest+testdst[ddx][0], sandboxtest+testsrc[sdx][0],testsize[sidx]);
				tested++;
				...
				if ((buffers_are_identical(sandboxorig, sandboxtest, SANDBOXSIZE) && (orig_ret - test_ret == sandboxorig - sandboxtest)))
				{
					printf("passed\n");
					passed++;
				}
